...
# --------------------------- MSH new ------------------------------------------
setwd("R:/GEMINI/_RESTORE/MSH/Clinical/")
files <- list.files(recursive = T)
files
msh.vitals <- NULL
for(i in 1:60){
  dat <- fread(files[i])
  msh.vitals <- rbind(msh.vitals, dat, fill = T)
}
...
ggplot(msh.dad[vitals==F], aes(ymd(Discharge.Date))) + geom_histogram(binwidth = 5)

fwrite(msh.vitals, "H:/GEMINI/Data/MSH/Vitals/msh.vitals.csv")
