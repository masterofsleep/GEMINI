...
crea.3d <- crea[ymd_hms(Collection.DtTm)<= ymd_hm(paste(Admit.Date, Admit.Time)) + days(3)&
                  ymd_hms(Collection.DtTm)>= ymd_hm(paste(Admit.Date, Admit.Time))]
base <- ddply(crea.3d, ~EncID.new, summarise,
              base.crea = min(Result.Value))

crea.max <- ddply(crea, ~EncID.new, summarise,
...
fwrite(inc, "C:/Users/guoyi/Desktop/to.adm/kdigo.csv")
table(inc$KDIGO)
1473/31006
